...
Pretty easy right? You've already seen most of this stuff plenty of times before so let's just talk about a few of the interesting bits. The Where clause lets you build your filter criteria using regular DBL syntax. Here's a few more valid where snippets to drive that home.
...
if you run this program the first time, you should see the following output:
...
There are some important things to keep in mind when using `Select` over a channel opened for update. First, you need to be aware that each record you read is going to be locked until you move to the next or finish the loop. If you dont intend to update records in the loop, you should use the `Select` class over a channel opened for input only. Next you need to know how to handle locked records, because there could be someone else with a lock on a record you're trying to read. 

## Events
In order to handle locked records you can implement a class that derives from the `Synergex.SynergyDE.Select.Event` class. This class has just one member, `onLock` which is called when a locked record is encountered by your Select object. Lets take a look at a basic implementation of this class:
...
This class is pretty simple but you can do a lot with those 4 arguments. The way you use the `lock` and `wait` parameters is actually to write out what you want the Select class to do on retry. You can set `wait` to 1 then return true if you want to retry with a wait similar to `WAIT: 1` in a `READS` statement. You get the contents of the record in `rec` and can use that to decide on your course of action. The `rfa` argument can be used to get the RFA of the record that was locked. You can store this RFA somewhere to try again later or offer up a list of exceptions to the user depending on what makes sense in your situation. If you want to skip the locked record you can set `lock` to `Q_NO_LOCK` or `Q_NO_DELETE` depending on if this is a read or a delete type operation. You can register your event handler with the `Select` object by calling `selectObj.RegisterEvent(new LockHandler())` prior to starting your `Delete()` or FOREACH loop.

## N + 1 Problem
The N + 1 query problem is a common performance issue in database operations, particularly evident in scenarios where a loop over a set of records (a driving table) triggers additional queries for each record. This problem is especially pertinent in an environment using xfServer.
